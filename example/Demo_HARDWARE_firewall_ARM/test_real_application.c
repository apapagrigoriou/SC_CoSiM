...
gid_t getBasicGroup()
{
	...
	gid_t gid_system_admin = groupIdFromName("root"); 
...
/* Function to list packet drops, if any
*/
void testStats(){
...
void setupFW_Final(){
	...
	// clinic/clinic1/clinic2 (mapped to inport 1/2/3) CAN respectively access BRAM1/2/3 for read-only
        //inport, L_addr_reg, H_addr_reg, rule, write_ops, read_ops, outport
...
int hashPatientNo(unsigned int patient_no)
{
	...
	//for(i=1; i<=1000; i++) { 
		//system("/bin/cat ./bla | md5sum | cut -d' ' -f1 > md5sum.txt");
		//system("echo $x");
		...
		//"\x70\x54\x9a\x6a\x7f\x6f\x56\x96" 
		//"\x1e\x84\xa5\x4b\xd9\x97\x0b\x8a", 256 / 8) != 0) { 
		//printf("SHA3-256(cc) " "doesn't match known answer (single buffer)\n"); 
	    	//} 
...
//for(i=0; i<1000; i++) {
	...
	// hash patient_no for location of patient data in BRAM (bram_no)
...
//}
	...
	/* 
	// Note: BRAM error exists!! (found above or equal to 0x80000100, 0x8000200, 0x8000300, 0x8000300)
	////N=1;
	////addr_reg = N*OFFSET_PATIENT_NO;
	////for (i=0; i<1024; i+=8){ // up to f8 it writing 4 bytes (unsigned integer) works...
	////{
	//keep this ... addr_reg = 0x000000fb;
	////printf("%x_________________\n",i);
	///}
	*/
...
//printf("Call setGidPerNoCInport_(%d_%d) \n", i, i%USER_NOC_INPUT_PORTS+1);
  		...
  		//printf("%d_Delay_setGidPerNoCInport: %lf sec (max:%lf, min:%lf) \n", i, avg_time/(1000*USER_NOC_INPUT_PORTS), max, min);
	}
...
#ifdef FW_USER_LOGS
//printf("Call setGidPerNoCOutport_(%d_%d) \n", i, i%USER_NOC_OUTPUT_PORTS+1);
#endif
			t2=(double)times(&tb); // end timer
      			avg_time+=(t2-t1)/ticspersec;
			max = t2-t1;
  		...
  		//printf("%d_Delay_setGidPerNoCOutport: %lf sec (max:%lf, min:%lf) \n", i, avg_time/(1000*USER_NOC_OUTPUT_PORTS), max, min);
	}
	...
	for(i=1; i<=1; i++) { 
		...
		for(i=0; i<NO_PATIENTS_IN_BRAMS; i++) {
			...
			// printf("setupPatientData%d mac:%d name:%d in different BRAM%d \n", 
