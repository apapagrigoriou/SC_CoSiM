...
struct simple_async_packet *shptr; //name of coSimulation data struct
...
void init_virtual_kernel(){
	...
	if(shptr==NULL){
        ...
        { 
			printf("Key File /tmp/sc_cosim.shm.key does not exists %s\n", strerror(errno));
            exit(-1);
        }
...
static int sc_accessBram(struct sc_direct_access_ds *my_direct_access_ds_ptr)
{
